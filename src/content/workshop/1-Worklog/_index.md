...
The diagram above maps every AWS service used in the AI Upscaler project to the week it was introduced. Traffic flows: **User → Route 53 → CloudFront (FE) / ALB (BE) → ECS on EC2 GPU (ASG capacity provider) → S3 / EFS / ElastiCache / SQS / CloudWatch / X-Ray**.
...
| ALB + AWS WAF + Cognito | TLS termination, CORS, WAF rate limits, OIDC auth | [Week 8](1.8-week8/) |
| CloudWatch Dashboard / SNS | Ops dashboard + alerting | [Week 9](1.9-week9/) |
| Secrets Manager · AWS WAF | Secret rotation + managed WAF rule set on CloudFront + ALB | [Week 10](1.10-week10/) |
| ECS on EC2 · ASG · SQS · ElastiCache Redis · EFS | Horizontal scale, async jobs, cache, shared model/DB storage | [Week 11](1.11-week11/) |
...
**Week 8:** [ALB + WAF + Cognito auth + integration tests](1.8-week8/)

**Week 9:** [Load test + tile inference + CloudWatch dashboard](1.9-week9/)

**Week 10:** [Secrets Manager + AWS WAF + API docs](1.10-week10/)

**Week 11:** [ECS on EC2 + ASG + ALB + SQS + Redis + EFS + Playwright E2E](1.11-week11/)
